<template>
  <a-form :model="formSearchParams" :style="{ marginBottom: '20px' }" layout="inline" @submit="doSearch">
  ...
  <a-table :columns="columns" :data="dataList" :pagination="{
    ...
    <template #reviewStatus="{ record }">
      {{ REVIEW_STATUS_MAP[record.reviewStatus as keyof typeof REVIEW_STATUS_MAP] }}
    </template>
    ...
    <template #optional="{ record }">
      <a-space>
        <a-button v-if="record.reviewStatus !== REVIEW_STATUS_ENUM.PASS" status="success"
          @click="doReview(record, REVIEW_STATUS_ENUM.PASS, '')">
          通过
        </a-button>
        <a-button v-if="record.reviewStatus !== REVIEW_STATUS_ENUM.REJECT" status="warning"
