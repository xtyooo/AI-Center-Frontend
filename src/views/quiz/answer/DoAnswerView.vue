<template>
  <div id="doAnswerPage">
    <a-card>
      ...
      <div>
        <a-radio-group direction="vertical" v-model="currentAnswer" :options="questionOptions as any"
      ...
      <div style="margin-top: 24px">
        <a-space size="large">
          ...
          <a-button type="primary" circle v-if="current < questionContent.length" :disabled="!currentAnswer"
            @click="current += 1">
            下一题
          </a-button>
          <a-button type="primary" v-if="current === questionContent.length" :loading="submitting" circle
            :disabled="!currentAnswer" @click="doSubmit">
            {{ submitting ? "评分中" : "查看结果" }}
          </a-button>
...
const generateId = function () {
  id.value = Math.floor(Math.random() * 1000000000);
}
