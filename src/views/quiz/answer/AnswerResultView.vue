    ...
    <a-card>
      <a-row style="margin-bottom: 16px">
        <a-col flex="auto" class="content-wrapper">
          ...
          <p>结果 id：{{ data.id }}</p>
          <p v-if="data.resultScore != null">结果得分：{{ data.resultScore}}</p>
          <p>我的答案：{{ data.choices }}</p>
          <p>应用 id：{{ data.appId }}</p>
          <p>应用类型：{{ APP_TYPE_MAP[data.appType as keyof typeof APP_TYPE_MAP] }}</p>
          <p>评分策略：{{ APP_SCORING_STRATEGY_MAP[data.scoringStrategy as keyof typeof APP_SCORING_STRATEGY_MAP] }}</p>
          <p>
            <a-space>
              答题人：
